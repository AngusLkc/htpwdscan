...
def gen_python_code(self):
    ...
    for param in self.args.d:
        ...
        if fname.startswith('md5(') and fname.endswith(')'):
            self.args.md5.append(pname)
            fname = fname[4: -1]
        elif fname.startswith('md5_16(') and fname.endswith(')'):
            self.args.md5_16.append(pname)
            fname = fname[7: -1]
        elif fname.startswith('sha1(') and fname.endswith(')'):
            self.args.sha1.append(pname)
            fname = fname[5: -1]
        self.selected_params[pname] =  fname
