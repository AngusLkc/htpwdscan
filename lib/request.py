...
def fake_ip(self, local_headers):
    if self.args.fip:
        local_headers['X-Forwarded-For'] = local_headers['Client-IP'] = '.'.join(str(random.randint(1,255)) for _ in range(4))
...
def fake_session_id(self, local_headers):
    if self.args.fsid:
        ...
        if m:
            ...
            local_headers['Cookie'] = local_headers['Cookie'].replace(m.group(0), '%s=%s' %(self.args.fsid, random_str))
        else:
        ...
        while err_count < max_err_count:
            try:
                if self.args.proxy_on:
                    ...
                    if self.args.get:
                        conn.request(method='GET', url='%s://%s/%s?%s'% (self.args.scm, self.args.netloc, self.args.path, data), headers=local_headers)
                    else:
                        conn.request(method='POST', url='%s://%s/%s' % (self.args.scm, self.args.netloc, self.args.path), body=data, headers=local_headers)
                else:
                ...
                if err_count == max_err_count: self.queue.put(origin_params)
